...
## Acceptance Criteria
- [x] `src/app/api/reviews/sweep/route.ts` exposes `POST /api/reviews/sweep` protected by the existing cron secret (same pattern as velocity cron from #029).
- [x] Sweep selects eligible rows (pending, linked, older than 24h), processes up to 50 per invocation, marks each `reviewed` or `discarded` (if `post_id` is still null after 7 days).
- [x] For each row: pull the post's metrics at 24h (likes, replies, reposts, shares, views) from existing `posts` and `posts_daily_stats` tables; store in `actual_windowed_metrics`.
- [x] Compute band verdict: `below_conservative` | `conservative` | `baseline` | `optimistic` | `above_optimistic` for each metric.
- [x] Invoke an LLM call via `resolveLLMClient(userId)` that uses knowledge prefix from #067 and produces a 2–4 sentence narrative: what beat/missed and the likely driver. Prompt lives at `src/lib/prompts/review-narrative.md`.
- [x] Write narrative into a new `narrative` text column on `post_predictions` (add in the migration for this ticket or extend #073's — prefer a new migration here to keep #073 minimal).
- [x] Supabase cron entry registers the sweep (hourly). Reuses the pattern from `supabase/migrations/20260322164000_supabase_cron_scheduler.sql`.

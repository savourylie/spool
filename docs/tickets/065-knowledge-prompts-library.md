...
## Acceptance Criteria
- [ ] `src/lib/prompts/` directory exists with five markdown files: `algorithm.md`, `psychology.md`, `ai-detection.md`, `data-confidence.md`, `README.md`.
- [ ] `algorithm.md` enumerates rules R1–R12 (engagement-bait, shortened view time, hashtag hygiene, etc.) and suppression risks S1–S14, each with a 1–2 sentence explanation; also documents the 4×4 post-publish matrix and first-3-hour strategy, rewritten in spool voice (not copied verbatim).
- [ ] `psychology.md` covers hook triggers (Information Gap, Zeigarnik, Peak-End), STEPPS framework, Cialdini's 7 principles, and ELM central/peripheral routes, with short actionable summaries.
- [ ] `ai-detection.md` lists the 10 sentence-level + 5 structure-level + 5 content-level AI-tone markers, each with one example and one remediation hint; plus the 6 de-AI-ification methods.
- [ ] `data-confidence.md` defines the five tiers (Directional <5, Weak 5–9, Usable 10–19, Strong 20–49, Deep 50+) and recommended UI treatment per tier.
- [ ] `README.md` explains the knowledge-as-data pattern, documents how tickets load these files, and includes MIT attribution to the AK-Threads-Booster source repo (`docs/seo/AK-Threads-booster/`).

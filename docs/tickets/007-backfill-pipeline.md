...
## Acceptance Criteria
- [ ] API route `POST /api/backfill/start` triggers backfill for the authenticated user
- [ ] Backfill job record updated to `running` status with `started_at` timestamp
- [ ] All user posts fetched via `ThreadsAPI.getUserPosts()` (paginated, from April 13 2024 onward)
- [ ] Repost facades excluded (handled by API service layer)
- [ ] For each post: insights fetched via `ThreadsAPI.getPostInsights()`
- [ ] Posts stored in `posts` table; initial metrics stored in `post_metrics` table
- [ ] `backfill_jobs.total_posts` set after post list fetch; `processed_posts` incremented as each post's insights are fetched
- [ ] User-level insights fetched: `followers_count`, demographics (country, city, gender — 3 calls)
- [ ] Initial `daily_stats` record created with current follower count
- [ ] Initial `demographics` records created
- [ ] Backfill job updated to `complete` with `completed_at` timestamp on success
- [ ] On failure: job updated to `failed` status, partial data preserved, error logged
- [ ] Rate limit awareness: respect 429 responses with backoff

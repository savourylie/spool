...
- [ ] Route secured with a `CRON_SECRET` header check (Vercel Cron pattern)
...
- [ ] Vercel cron config in `vercel.json` with 6-hour schedule
...
- For local dev: use `pg_cron` or manual trigger via `curl`

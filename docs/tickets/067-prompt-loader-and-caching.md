...
## Acceptance Criteria
- [x] New util `src/lib/prompts/loader.ts` exports `loadPrompt(name: string): string` that reads and caches markdown files from `src/lib/prompts/` at module-load time (server-side only).
- [x] `quality-llm.ts` composes its system prompt from: `loadPrompt("algorithm")` + `loadPrompt("psychology")` + `loadPrompt("ai-detection")` + user-variable suffix.
- [x] `composer-prompt.ts` composes its system prompt from: `loadPrompt("algorithm")` + `loadPrompt("psychology")` + user-variable suffix.
- [x] Claude API calls use `cache_control: { type: "ephemeral" }` on the knowledge-prefix message block; the user-variable block is uncached.
- [x] OpenAI BYOK path (via `openai-client.ts`) continues to work — it receives the full composed prompt without `cache_control` (OpenAI doesn't support it); add a branch in the LLM resolver path to skip cache markers for non-Anthropic providers.
- [x] A small runtime log at the end of each Anthropic call captures `cache_creation_input_tokens` and `cache_read_input_tokens` from the response so ticket #082 can measure hit rate.

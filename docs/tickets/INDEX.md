...
> **Last updated**: 2026-04-23 (TICKET-068 done, TICKET-069/070 unblocked) <!-- updated by /update-ticket -->
...
| #   | Ticket                                                                          | Status    | Depends On   | Notes |
| --- | ------------------------------------------------------------------------------- | --------- | ------------ | ----- |
| 065 | [Knowledge Prompts Library](./065-knowledge-prompts-library.md)                 | `done`    | —            | `src/lib/prompts/` with algorithm, psychology, ai-detection, data-confidence |
| 066 | [Data Confidence Util + Badge](./066-data-confidence-util-and-badge.md)         | `done`    | —            | 5-tier util + ConfidenceBadge primitive; retrofit across Understand + Insights + Today Hub |
| 067 | [Prompt Loader Refactor + Caching](./067-prompt-loader-and-caching.md)         | `done`    | #065 ✅      | Scanner + Composer load from `src/lib/prompts/`; Anthropic cache_control breakpoints |

### v3 Phase 2 — Brand Voice (068-070)

| #   | Ticket                                                                          | Status    | Depends On       | Notes |
| --- | ------------------------------------------------------------------------------- | --------- | ---------------- | ----- |
| 068 | [Brand Voice Extraction Infra](./068-brand-voice-extraction-infra.md)           | `done`    | #067 ✅          | `brand_voice_profiles` table, `/api/brand-voice/refresh` endpoint, 11-dimension extraction |
| 069 | [Brand Voice UI Panel](./069-brand-voice-ui-panel.md)                           | `pending` | #066 ✅, #068 ✅    | Unblocked — `/dashboard/understand/voice` accordion with excerpts + refresh button |
| 070 | [Brand Voice Composer + Scanner Wiring](./070-brand-voice-composer-scanner-wiring.md) | `pending` | #068 ✅    | Unblocked — primary driver in Composer, observer-only in Scanner (discipline codified) |
...
| #   | Ticket                                                                          | Status    | Depends On   | Notes |
| --- | ------------------------------------------------------------------------------- | --------- | ------------ | ----- |
| 071 | [Freshness Gate Infra](./071-freshness-gate-infra.md)                           | `pending` | #065 ✅, #067 ✅ | Unblocked — `freshness_checks` table, combined external + self-repetition check, Composer banner |
...
| #   | Ticket                                                                          | Status    | Depends On         | Notes |
| --- | ------------------------------------------------------------------------------- | --------- | ------------------ | ----- |
| 073 | [Post Predictions Persistence](./073-post-predictions-persistence.md)           | `pending` | #066 ✅               | Unblocked — `post_predictions` table, snapshot on compose, fuzzy-match link-back in backfill |
...
| #   | Ticket                                                                          | Status    | Depends On     | Notes |
| --- | ------------------------------------------------------------------------------- | --------- | -------------- | ----- |
| 077 | [Scanner Four-Axis Backend](./077-scanner-four-axis-backend.md)                 | `pending` | #065 ✅, #067 ✅ | Unblocked — 4-axis output shape, `analyze.md` prompt, neighbor-post citations, feature flag |
...
| 080 | [Concept Library Infra](./080-concept-library-infra.md)                         | `pending` | #065 ✅, #067 ✅ | Unblocked — `concept_ledger` table, extraction classifier, reuse-risk aggregate |
...
```
Phase 1 (foundation):
  065 Knowledge Prompts ✅ DONE ──┬──► 067 Prompt Loader ✅ DONE ──┐
  066 Confidence Util + Badge ✅ DONE (standalone)                  │
                                                                     │
Phase 2 (brand voice):                                               │
  067 ✅ DONE ─► 068 Brand Voice Infra ✅ DONE ──┬──► 069 Brand Voice UI 📋 PENDING
  066 ✅ DONE ──────────────────────────────────┘
  068 ✅ DONE ─────────────────────────────► 070 Composer + Scanner Wiring 📋 PENDING

Phase 3 (freshness):
  065 ✅ DONE ─┬─► 071 Freshness Gate Infra 📋 PENDING ──► 072 Today Hub Filter
  067 ✅ DONE ─┘

Phase 4 (review loop):
  066 ✅ DONE ─► 073 Predictions Persistence 📋 PENDING ──┬──► 074 Review Sweep ──┬──► 075 Reviews Page
  067 ✅ DONE ────────────────────────────────────────────┘                       ├──► 076 Latest Review Card
  072 ───────────────────────────────────────────────────────────────────────────┘

Phase 5 (diagnostic polish):
  065 ─┬─► 077 Scanner 4-axis Backend 📋 PENDING ──► 078 4-axis UI ──► 079 AI-Tone Markers
  067 ✅ DONE ─┤
       └─► 080 Concept Library Infra 📋 PENDING ──► 081 Concept Library UI

# Ticket Index

> **Last updated**: 2026-03-15
> Auto-generated by Claude Code. Update this file whenever a ticket status changes.

## Summary

| Status        | Count |
| ------------- | ----- |
| ✅ Done        | 0     |
| 🔧 In Progress | 0     |
| 📋 Pending     | 1     |
| 🚫 Blocked     | 21    |
| ⏸️ Deferred    | 0     |

---

## Phase 1 — Foundation

| #   | Ticket                                                           | Status    | Depends On | Notes |
| --- | ---------------------------------------------------------------- | --------- | ---------- | ----- |
| 001 | [Project Scaffolding & Design Tokens](./001-project-scaffolding.md) | `pending` | —          |       |
| 002 | [Database Schema & Supabase Config](./002-database-schema.md)    | `blocked` | #001       |       |
| 003 | [Shared UI Components](./003-shared-ui-components.md)            | `blocked` | #001       |       |

## Phase 2 — Auth & API Layer

| #   | Ticket                                                           | Status    | Depends On | Notes |
| --- | ---------------------------------------------------------------- | --------- | ---------- | ----- |
| 004 | [Threads OAuth Flow](./004-threads-oauth.md)                     | `blocked` | #002       |       |
| 005 | [Token Encryption](./005-token-encryption.md)                    | `blocked` | #004       |       |
| 006 | [Threads API Service Layer](./006-threads-api-service.md)        | `blocked` | #001       |       |

## Phase 3 — Data Ingestion

| #   | Ticket                                                           | Status    | Depends On   | Notes |
| --- | ---------------------------------------------------------------- | --------- | ------------ | ----- |
| 007 | [Backfill Pipeline](./007-backfill-pipeline.md)                  | `blocked` | #002, #005, #006 |   |
| 008 | [Backfill Progress UI](./008-backfill-progress-ui.md)            | `blocked` | #003, #007   |       |
| 009 | [Scheduled Metrics Refresh](./009-scheduled-metrics-refresh.md)  | `blocked` | #002, #006   |       |
| 010 | [Scheduled Daily Stats & Demographics](./010-scheduled-daily-stats.md) | `blocked` | #002, #006 |  |
| 011 | [Scheduled Token Refresh](./011-scheduled-token-refresh.md)      | `blocked` | #005, #006   |       |

## Phase 4 — Frontend Pages

| #   | Ticket                                                           | Status    | Depends On   | Notes |
| --- | ---------------------------------------------------------------- | --------- | ------------ | ----- |
| 012 | [Landing Page](./012-landing-page.md)                            | `blocked` | #003         |       |
| 013 | [Dashboard Layout & Tab Navigation](./013-dashboard-layout.md)   | `blocked` | #003, #004   |       |
| 014 | [Post Performance Table](./014-post-performance-table.md)        | `blocked` | #002, #013   |       |
| 015 | [Post Table Filters & Sorting](./015-post-filters.md)            | `blocked` | #014         |       |
| 016 | [Post Detail Expansion & Sparkline](./016-post-detail-expansion.md) | `blocked` | #014      |       |
| 017 | [Best Time to Post Heatmap](./017-best-time-heatmap.md)          | `blocked` | #002, #013   |       |
| 018 | [Follower Trend Line Chart](./018-follower-trend-chart.md)       | `blocked` | #002, #013   |       |
| 019 | [Demographics Charts](./019-demographics-charts.md)              | `blocked` | #002, #013   |       |

## Phase 5 — Polish

| #   | Ticket                                                           | Status    | Depends On       | Notes |
| --- | ---------------------------------------------------------------- | --------- | ---------------- | ----- |
| 020 | [Edge Case & Empty States](./020-edge-cases-empty-states.md)     | `blocked` | #014, #017, #018, #019 |       |
| 021 | [Responsive & Accessibility Pass](./021-responsive-accessibility.md) | `blocked` | #012–#020    |       |
| 022 | [QA & Final Polish](./022-qa-final-polish.md)                    | `blocked` | All              |       |

---

## Dependency Graph

```
001 Project Scaffolding & Design Tokens
 ├── 002 Database Schema & Supabase Config
 │    ├── 004 Threads OAuth Flow ──► 005 Token Encryption
 │    │                                    │
 │    │    006 Threads API Service ◄───────┘
 │    │    ├── 007 Backfill Pipeline ──► 008 Backfill Progress UI
 │    │    ├── 009 Scheduled Metrics Refresh
 │    │    ├── 010 Scheduled Daily Stats & Demographics
 │    │    └── 011 Scheduled Token Refresh
 │    │
 │    ├── 014 Post Performance Table
 │    ├── 017 Best Time Heatmap
 │    ├── 018 Follower Trend Chart
 │    └── 019 Demographics Charts
 │
 ├── 003 Shared UI Components
 │    ├── 008 Backfill Progress UI
 │    ├── 012 Landing Page
 │    └── 013 Dashboard Layout ──► 014, 015, 016, 017, 018, 019
 │
 └── All ──► 020 Edge Cases ──► 021 Responsive & A11y ──► 022 QA/Polish
```

---

## Status Key

| Status        | Meaning                                              |
| ------------- | ---------------------------------------------------- |
| `pending`     | Ready to be picked up — all dependencies met          |
| `in-progress` | Currently being implemented                           |
| `done`        | Implemented and verified                              |
| `blocked`     | Cannot start — waiting on dependency                  |
| `deferred`    | Intentionally postponed (reason noted in Notes column)|

...
## Acceptance Criteria
- [x] `WhatToPostCard` — shows 2-3 topic suggestions with colored dots, topic text, and a "Go compose" pill button linking to `/dashboard/create/compose?topic={encoded}`
- [x] `WhenToPostCard` — shows "NEXT BEST SLOT" with day/time, avg engagement; a mini 3×7 heatmap (hour blocks grouped into AM/PM/Evening); cadence status indicator (on track / due / overdue)
- [x] `PulseCard` — shows 7-day delta for followers (+N), post count, avg engagement rate, and velocity trend (up/down/flat arrow with color)
- [x] `BestPostCard` — shows top WES post preview (truncated), metrics (views, likes, replies), and "Why it worked" section listing format, timing, and topic factors
- [x] All cards use `StickerCard` or consistent card styling (rounded-16, white fill, border, padding-24)
- [x] All cards have proper loading skeletons
- [x] All cards have empty states for new accounts with no data

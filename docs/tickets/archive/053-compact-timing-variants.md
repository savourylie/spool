...
## Acceptance Criteria
- [x] `TimingHeatmap` accepts optional `compact?: boolean` prop
- [x] Compact heatmap renders a 7×6 grid (days × 4-hour blocks: 12a-4a, 4a-8a, 8a-12p, 12p-4p, 4p-8p, 8p-12a)
- [x] Compact heatmap averages engagement across the 4 hours in each block
- [x] Compact heatmap omits the timezone selector dropdown
- [x] `CadenceOptimizer` accepts optional `compact?: boolean` prop
- [x] Compact cadence shows: status indicator (green/yellow/red dot), one-line summary ("Optimal spacing: 18-24h"), last post time, and "See full analysis →" link
- [x] Compact cadence omits the scatter chart and collision list
- [x] Full (non-compact) versions remain unchanged — no regressions
- [x] Best-3-times list extracted as standalone section usable in compact mode

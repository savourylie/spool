...
export function BrandVoicePanel({
  ...
  if (!record) {
    return (
      <div className="py-8">
        <h1 className="font-heading text-3xl font-medium">Your Brand Voice</h1>
  ...
  return (
    <div className="py-8">
      ...
      {isDirectional && (
        <div
          role="status"
          className="mt-6 rounded-[var(--radius-md)] border border-tertiary bg-tertiary/25 px-4 py-3"
        >
                ...
                {evidence.length === 0 ? (
                  ...
                  evidence.map((ev, evIdx) => {
                    ...
                    return (
                      <blockquote
                        key={`${ev.postId}-${evIdx}`}
                        className="rounded-[var(--radius-sm)] border border-border bg-muted/30 px-4 py-3"
                      >
